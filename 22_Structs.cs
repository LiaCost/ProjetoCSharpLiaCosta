...
namespace Curso_C_
{
    ...
    class: É um tipo de referência. Quando você cria uma instância de uma classe, você está criando um objeto que é armazenado no heap. 
           Variáveis de classe armazenam referências ao objeto, não o próprio objeto.
    struct: É um tipo de valor. Quando você cria uma instância de uma struct, ela é armazenada na stack (ou no heap, 
            se for um campo de uma classe) e as variáveis de struct contêm diretamente o valor.

    Herança:

    class: Suporta herança. Você pode criar uma classe base e derivar outras classes dela.
    struct: Não suporta herança. Estruturas não podem ser base para outras structs ou classes e 
            não podem derivar de outras structs ou classes (exceto para System.ValueType).
    Construtores e Destrutores:

    class: Pode ter construtores personalizados e destrutores (finalizadores).
    struct: Tem um construtor padrão implícito (que inicializa todos os campos com valores padrão) 
           e você pode definir construtores parametrizados, mas não pode ter um destruidor.

    Inicialização e Cópia:

    class: Quando você copia uma variável de classe, você copia a referência, não o objeto em si. Portanto, 
    alterações em uma instância afetam todas as variáveis que referenciam essa instância.
    struct: Quando você copia uma variável de struct, você copia o valor inteiro. 
    Isso significa que alterações em uma cópia não afetam a instância original.
    ...
    struct: Usado para objetos pequenos e imutáveis que são frequentemente criados e destruídos, como vetores e coordenadas. 
    Estruturas podem ter desempenho melhor para esses casos, mas usar structs grandes pode levar a uma maior sobrecarga de cópias.
